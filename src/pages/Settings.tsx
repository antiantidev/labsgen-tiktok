  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-10 pb-20">
      <div className="flex items-start justify-between gap-6">
        ...
        <Button className="h-12 px-6 rounded-xl" onClick={() => void saveConfig(true)} icon={Save}>
        ...
        <div className="xl:col-span-8 space-y-10">
          {isDriverMissing && (
            <AlertBanner
              ...
              actions={
                <Button variant="primary" onClick={() => void handleInstallDriver()} loading={isInstallingDriver} className="h-10 px-6 rounded-xl text-xs" icon={Download}>
                ...
                <div className="flex gap-2">
                  {['vi', 'en'].map((lang) => (
                    <button
                      ...
                      className={`flex-1 h-12 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all border ${
                        i18n.language.startsWith(lang) ? 'bg-primary text-primary-foreground border-primary' : 'bg-secondary border-border light:bg-white/70 light:border-black/5 hover:border-primary/30'
                      }`}
                    >
                      {lang.toUpperCase()}
                    </button>
            ...
            <div className="space-y-6">
              <div className="p-6 rounded-xl bg-secondary/50 border border-border light:bg-white/70 light:border-black/5 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" className="h-10 w-10 p-0 rounded-lg" onClick={() => void handlePathChange()} title={t('settings.change')} icon={Edit3} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  >
                    <div className="p-2.5 rounded-lg bg-secondary text-muted-foreground group-hover:text-primary transition-colors">
                      <item.icon size={16} />
                    </div>
        ...
        <div className="xl:col-span-4 space-y-10">
          {!isDriverMissing && (
            <Card title={t('settings.driver_status')}>
              ...
              <Button variant="outline" onClick={() => void handleInstallDriver()} loading={isInstallingDriver} className="w-full mt-6 h-12 text-xs">
          ...
          <Card title={t('settings.update_check')}>
            <div className="space-y-6 text-center">
              <div className={`w-20 h-20 rounded-[24px] flex items-center justify-center mx-auto ${isUpToDate ? 'bg-success/10 text-success' : 'bg-primary/10 text-primary'}`}>
                {isUpToDate ? <CheckCircle2 size={32} /> : <RefreshCw size={32} className={checkingUpdate ? 'animate-spin' : ''} />}
              </div>
              <div>
              ...
              {updateProgress && updateProgress.percent > 0 && (
                <div className="space-y-2">
                  <div className="h-2 w-full bg-secondary rounded-full overflow-hidden border border-white/10 light:border-black/10">
                    <div className="h-full bg-primary transition-all" style={{ width: `${Math.min(100, Math.max(0, updateProgress.percent))}%` }} />
                  </div>
              ...
              <Button className="w-full h-14 text-xs" loading={checkingUpdate} onClick={() => void handleUpdateCheck()}>
          ...
          <Card title={t('settings.app_info')}>
            <div className="space-y-4">
              ...
              <Button variant="outline" className="w-full mt-4 h-12 text-xs" onClick={() => api.openExternal("https://github.com/antiantidev/labsgen-tiktok")}>
