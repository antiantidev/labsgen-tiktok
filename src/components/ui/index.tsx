...
export const Button = ({ children, onClick, disabled, variant = "primary", className = "", icon: Icon, loading, title }: ButtonProps) => {
  const variants: Record<ButtonVariant, string> = {
    primary:
      "bg-[linear-gradient(135deg,hsl(var(--primary)),hsl(var(--primary))_55%,rgba(255,255,255,0.08))] text-primary-foreground shadow-[0_12px_32px_rgba(34,197,94,0.25)] hover:shadow-[0_18px_40px_rgba(34,197,94,0.35)] ring-1 ring-primary/30",
...
export const Card = ({ children, title, className = "", onClick }: CardProps) => (
  ...
  >
    {title && (
      <h3 className="text-[10px] font-black uppercase tracking-[0.25em] text-muted-foreground/80 mb-5 group-hover:text-primary transition-colors">
        {title}
      </h3>
    )}
    {children}
...
export const Input = ({ label, icon: Icon, className = "", ...props }: InputProps) => (
  <div className="group space-y-2 w-full">
    {label && (
      <label className="text-[10px] uppercase font-bold text-muted-foreground/80 ml-1 group-focus-within:text-primary transition-colors tracking-[0.2em]">
        {label}
      </label>
    )}
...
export const Switch = ({ checked, onChange, label, description, icon: Icon }: SwitchProps) => (
  <label className="flex items-center justify-between p-5 rounded-2xl bg-secondary/50 border border-border hover:border-primary/30 transition-all cursor-pointer group">
    ...
    <div className="relative inline-flex items-center shrink-0">
      <input type="checkbox" className="sr-only" checked={checked} onChange={(e) => onChange(e.target.checked)} />
      <motion.div animate={checked ? { backgroundColor: "hsl(var(--primary))" } : { backgroundColor: "hsl(var(--muted))" }} className="w-11 h-6 rounded-full transition-colors relative">
        <motion.div
...
export const Skeleton = ({ className = "" }: { className?: string }) => <div className={`skeleton ${className} rounded-lg`} />
...
export const LoadingOverlay = ({ message, progress }: LoadingOverlayProps) => (
  <motion.div
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
    exit={{ opacity: 0, scale: 1.05, filter: "blur(20px)", transition: { duration: 0.8, ease: [0.43, 0.13, 0.23, 0.96] } }}
    ...
    <motion.div initial={{ y: 0 }} exit={{ y: -40, opacity: 0, transition: { duration: 0.6 } }} className="flex flex-col items-center">
      ...
      <div className="w-64 space-y-4">
        ...
        <div className="flex flex-col items-center gap-1">
          <motion.p key={message} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="text-[10px] font-black uppercase tracking-[0.3em] text-primary/80">
            {message}
          </motion.p>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-[13px] font-bold leading-snug text-center px-2 ${type === "error" ? "text-destructive-foreground" : "text-foreground"}`}>{message}</p>
      </div>
      <button
        onClick={(e) => {
          e.stopPropagation()
        ...
        className="p-2 -mr-1 rounded-lg hover:bg-white/10 light:hover:bg-black/5 text-muted-foreground hover:text-foreground transition-all shrink-0"
        aria-label="Close"
      >
        <X size={16} />
      </button>
